...
if (cluster.isMaster) {
  ...
  if (args.length < 3) {
    console.error('Usage: node l.js <target_url> <duration_seconds> <threads> [proxy_file]');
    process.exit(1);
  }
  ...
  if (args[3]) {
    try {
      proxies = fs.readFileSync(args[3], 'utf8').split(/
?
/).filter(Boolean);
    } catch (e) {
  ...
  const proxies = process.argv[5] ? fs.readFileSync(process.argv[5], 'utf8').split(/
?
/).filter(Boolean) : [];
